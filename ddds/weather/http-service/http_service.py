...
def get_data(city,country, unit="metric"):
    url = f"http://api.openweathermap.org/data/2.5/weather?q={city},{country}&units={unit}&APPID=1d8bb92f4ccb7ede3804fd59bdbfee4e"
    print(url)
    request = Request(url)
...
def temperature():
    ...
    unit_check = payload["context"]["facts"]
    city = payload["context"]["facts"]["city_to_search"]["grammar_entry"]
    fixedcity=city.replace(" ", "%20")
    country = payload["context"]["facts"]["country_to_search"]["grammar_entry"]
    fixedcountry = country.replace(" ", "%20")
    if "unit_to_search" in unit_check:
        unit = unit_check["unit_to_search"]["grammar_entry"]
    else:
        unit = "metric"
    data = get_data(fixedcity, fixedcountry, unit)
    temp = data['main']['temp']
    tempstr = str(temp)
